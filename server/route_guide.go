...
var (
	...
	jsonDBFile = flag.String("json_db_file", "database/testdata/route_guide_db.json", "A json file containing a list of features")
	port       = flag.Int("port", 10000, "The server port")
)

var _ rg.RouteGuideServer = &routeGuideServer{}

type routeGuideServer struct {
	savedFeatures []*rg.Feature // read-only after initialized

	mu         sync.Mutex // protects routeNotes
	routeNotes map[string][]*rg.RouteNote
}

// GetFeature returns the feature at the given point.
func (s *routeGuideServer) GetFeature(ctx context.Context, point *rg.Point) (*rg.Feature, error) {
	...
	return &rg.Feature{Location: point}, nil
}

// ListFeatures lists all features contained within the given bounding Rectangle.
func (s *routeGuideServer) ListFeatures(rect *rg.Rectangle, stream rg.RouteGuide_ListFeaturesServer) error {
	for _, feature := range s.savedFeatures {
...
// total time spent.
func (s *routeGuideServer) RecordRoute(stream rg.RouteGuide_RecordRouteServer) error {
	var pointCount, featureCount, distance int32
	var lastPoint *rg.Point
	startTime := time.Now()
	for {
		point, err := stream.Recv()
		if err == io.EOF {
			endTime := time.Now()
			return stream.SendAndClose(&rg.RouteSummary{
...
func (s *routeGuideServer) RouteChat(stream rg.RouteGuide_RouteChatServer) error {
	for {
		in, err := stream.Recv()
		if err == io.EOF {
			return nil
		...
		rn := make([]*rg.RouteNote, len(s.routeNotes[key]))
...
func calcDistance(p1 *rg.Point, p2 *rg.Point) int32 {
...
func inRange(point *rg.Point, rect *rg.Rectangle) bool {
...
func serialize(point *rg.Point) string {
